...
export function APAgingPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['ap-aging', legalEntityId, asOfDate],
    queryFn: () => apApi.getAPAging({ legal_entity_id: legalEntityId, as_of_date: asOfDate }),
  ...
  if (!legalEntityId) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg">
        Select an entity to view AP aging.
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
        Error loading AP aging. Please try again.
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Outstanding payables by age</p>
        </div>
        <input
          type="date"
          value={asOfDate}
