...
================================================================================
8) NO “HIDE FAILURES” FIXES (FORBIDDEN)
================================================================================
Forbidden unless user explicitly approves AND status remains UNVERIFIED:
- Setting typescript.ignoreBuildErrors = true
- Commenting out tests
- Skipping migrations
- Bypassing auth globally
- Removing failing checks without replacement
- Replacing real logic with stubs but calling it “done”

If stubs are added temporarily:
- Must be labeled STUB clearly
- Must list them in checkpoint
- Status cannot be DONE

================================================================================
9) NO “ENVIRONMENT IS BROKEN” CLAIMS WITHOUT PROOF
================================================================================
An agent may NOT claim “Windows/Python/pip is broken” unless they provide:

A) Minimal repro OUTSIDE repo:
- create a folder: C:\dev\pycheck
- create venv
- run ensurepip
- import pip
...
B) Evidence:
- Ctrl+C stack trace OR clear observable stall point
- log output captured

C) Workaround executed:
- Use Python 3.11 if project stack expects it (or user-approved version)
- Re-run truth commands after workaround
If workaround not executed => status stays UNVERIFIED (not BLOCKED)

================================================================================
10) GIT HYGIENE + PUSH PROTOCOL
================================================================================
10.1 No push without permission:
- Agent must not push to origin unless user explicitly says “push now”
- Agent may prepare commits locally

10.2 If user expects push (and grants permission), DONE requires:
- git status
- git log -1 --oneline
- git remote -v
- git push output
...
Use concrete commits:
- feat: <feature>
- fix: <bug>
- chore: <maintenance>
No vague “update” commits.

================================================================================
11) PER-REPO ADDENDUM (REQUIRED FOR STRUCTURE + NON-NEGOTIABLES)
================================================================================
Global rules are generic. Each repo SHOULD have:
- docs/01-main/REPO_RULES_ADDENDUM.md

Addendum must include:
- Repo type (Python/Node/Monorepo)
- Truth command set for that repo (exact commands)
- Directory structure constraints (where new modules go)
- Any non-negotiables (security, accounting invariants, etc.)
- The “do not invent folders” rule for that repo

Addendum may tighten rules, but may NOT weaken:
- Status words limitation
- Truth command requirement for DONE
- Output capture requirement
- First-failure fix loop

================================================================================
12) REQUIRED END-OF-MESSAGE COMPLETION CHECKLIST (EVERY TIME)
================================================================================
At the end of any “final” message, agent MUST include:
...
- YES (pasted) OR YES (logs/<file>.log) OR NO
REMAINING FAILURES:
- none OR list
NEXT ACTION:
- single next command to run

If any of these are missing, response is invalid.
